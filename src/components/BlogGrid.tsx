      ...
      {posts.map((post) => (
        ...
        >
          <Link href={`/blog/${post.slug}`} className="block">
            ...
            {/* Text */}
            <div className="flex flex-1 flex-col px-7 pt-7 pb-8 md:px-9 md:pt-8 md:pb-9">
              {/* Caption meta */}
              ...
              {/* Title */}
              <h3 className="mt-4 h-[2lh] text-[20px] md:text-[22px] leading-snug tracking-[-0.01em]">
                {post.title}
              </h3>

              {/* Excerpt */}
              {post.excerpt ? (
                <p className="mt-4 h-24 text-[15px] leading-relaxed text-charcoal/80 line-clamp-4">
                  {post.excerpt}
                </p>
              ) : null}
